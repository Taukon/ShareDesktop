...
export class FileShare {
    ...
    public async sendStreamFile(
        ...
        if(filePath && !this.isWritingFile(fileName)){
            ...
            for await (const chunk of fileReadStream){
                ...
                await timer(10);
                fileWindow.webContents.send('streamSendFileBuffer', {fileTransferId: fileTransferId, buf: chunk});
            }
    ...
    private watchChange(watcher: chokidar.FSWatcher, fileWindow: BrowserWindow, basePath: string) {
        watcher.on("ready", () => {
    ...
    public sendFilelist(fileWindow: BrowserWindow): boolean{
    ...
    private watchlist(watcher: chokidar.FSWatcher, fileWindow: BrowserWindow): boolean {
