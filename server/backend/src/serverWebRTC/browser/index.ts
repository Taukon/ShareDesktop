...
export class Browser {
  ...
  private isBrowserList(
    browserList: BrowserList | undefined,
  ): browserList is BrowserList {
  ...
  public disconnectBrowserClient(browserId: string): void {
    ...
    if (browserList?.exits) {
      Object.entries(browserList).map(([, browserTransports]) => {
  ...
  public verifyTotal(limitBrowser: number): string | undefined {
    console.log(
      "Total Client List: " + Object.keys(this.browserClientList).length,
    );
